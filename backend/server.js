...
await mongoose.connect('mongodb+srv://admin:admin@greddiit.y8pu7ok.mongodb.net/?retryWrites=true&w=majority', {useNewUrlParser:true,useUnifiedTopology:true,});
...
app.post('/register', (req, res) => {
  const {email,username, firstName, lastName, age, contact} = req.body;
  const password = bcrypt.hashSync(req.body.password, 10);
  const user = new User({email,username,password, firstName, lastName, age, contact});
  user.save().then(user => {
