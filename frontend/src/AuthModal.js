...
function AuthModal() {
  ...
  function register(e) {
    e.preventDefault();
    const data = {email,username,password, firstName, lastName, age, contact};
      ...
      <ClickOutHandler onClickOut={() => modalContext.setShow(false)}>
        <div className="border border-reddit_dark-brightest w-3/4 sm:w-1/2 lg:w-1/4 bg-reddit_dark p-5 text-reddit_text self-center mx-auto rounded-md">
          ...
          {modalType === 'register' && (
            ...
            <label>
              <span className="text-reddit_text-darker text-sm">First Name:</span>
              <Input type="text" className="mb-3 w-full" value={firstName} onChange={e => setFirstName(e.target.value)} />
            </label>

            <label>
              <span className="text-reddit_text-darker text-sm">Last Name:</span>
              <Input type="text" className="mb-3 w-full" value={lastName} onChange={e => setLastName(e.target.value)} />
            </label>

            <label>
              <span className="text-reddit_text-darker text-sm">Age:</span>
              <Input type="text" className="mb-3 w-full" value={age} onChange={e => setAge(e.target.value)} />
            </label>

            <label>
              <span className="text-reddit_text-darker text-sm">Contact:</span>
              <Input type="text" className="mb-3 w-full" value={contact} onChange={e => setContact(e.target.value)} />
            </label>
