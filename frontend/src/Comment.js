...
function Comment(props) {
  ...
  function refreshComments() {
    axios.get('http://localhost:8000/comments/root/'+props.id)
      .then(response => {
  ...
  function refreshVotes() {
    ...
    axios.post('http://localhost:8000/votes', {commentsIds}, {withCredentials:true})
      .then(response => {
  ...
  useEffect(() => {
    if (props.comment) {
      ...
      axios.get('http://localhost:8000/comments/'+props.id)
        .then(response => {
